...
function safeSelf() {
    ...
    const safe = {
        ...
        uboLog(...args) {
            return this.sendToLogger && this.sendToLogger('info', ...args);
            
        },
        uboErr(...args) {
            return this.sendToLogger && this.sendToLogger('error', ...args);
        },
        escapeRegexChars(s) {
            return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        },
    ...
    if ( scriptletGlobals.bcSecret !== undefined ) {
        ...
        let lastLoggedType = '';
        let lastLoggedText = '';
        let lastLoggedTextCount = 0;
        scriptletGlobals.shouldLog = true;
        safe.sendToLogger = (type, ...args) => {
            if ( args.length === 0 ) { return; }
            const text = `[${document.location.hostname || document.location.href}]${args.join(' ')}`;
            if ( type === lastLoggedType && text === lastLoggedText ) {
                lastLoggedTextCount += 1;
                if ( lastLoggedTextCount < 10 ) { return; }
            }
            if ( lastLoggedTextCount !== 0 ) {
                if ( lastLoggedTextCount !== 1 ) {
                    lastLoggedText = `[${lastLoggedTextCount}\u00D7]${lastLoggedText}`;
                }
                bc.postMessage({ what: 'messageToLogger', type: lastLoggedType, text: lastLoggedText });
                lastLoggedTextCount = 0;
            }
            lastLoggedType = type;
            lastLoggedText = text;
...
function abortCurrentScriptCore(
    ...
    const validate = ( ) => {
        ...
        safe.uboLog(logPrefix, 'Aborting');
...
function hrefSanitizer(
    ...
    const sanitize = ( ) => {
        ...
        for ( const elem of elems ) {
            ...
            safe.uboLog(logPrefix, `Sanitized ${count+1} links to ${hrefAfter}`);
        }
        return true;
    };
    let observer, timer;
